...
## -*- texinfo -*- 
## @deftypefn {} {@var{retval} =} genequalareacountnet (@var{input1}, @var{input2})

## Generates the location points of an rectangular equal area counting net.
...
function [coneAxisTrendPlungeMat] = genequalareacountnet1 ...
    (countingCircleAreaInPercent, wantplot, projType)
...
%CREATING THE INSCRIBED POLAR MATRIX
indexes =polPointsMat(:,2) <=1;
inscribedPolPtsMat =polPointsMat(indexes,:);
inscribedPolPtsMat =sortorientations( inscribedPolPtsMat );

%CREATING THE TREND-PLUNGE MATRIX
%Back calculation of trend-plunge from polar matrix
coneAxisTrendPlungeMat =planepolar2equalareatrendplunge_array( inscribedPolPtsMat );
coneAxisTrendPlungeMat =sortorientations( coneAxisTrendPlungeMat );

## Plotting the centers of the couting net.
if wantplot
    hold on
